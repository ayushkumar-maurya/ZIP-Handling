...
def main(parent):
	# Home Content
	content_frame = Frame(parent, bg=attr_bg2)
	...
	app_logo_label = Label(content_frame, image=app_logo_img_tk, bg=attr_bg2)
	app_logo_label.image = app_logo_img_tk
	app_logo_label.pack()

	Label(content_frame, text=app_desc1, bg=attr_bg2).pack(fill="x", pady=(30, 0))
	Label(content_frame, text=app_desc2, bg=attr_bg2).pack(fill="x", pady=(60, 0))

	# Footer
	footer_frame = Frame(parent, bg=attr_bg2)
	footer_frame.grid(row=1, column=0, pady=(10, 0), sticky="ew")
	Label(footer_frame, text="Developed by Ayushkumar Maurya", bg=attr_bg2).pack()
...
app_desc1 += "Copy files within ZIP"
