...
def perform_copy_operation(entries, show_progress_bar):
	show_progress_bar()
...
def main(parent, show_progress_bar):
	...
	get_label(parent, "File to copy (Relative path)", False).grid(row=2, column=0, sticky="ew")
	...
	copy_btn = get_submit_btn(parent, "Copy")
	copy_btn.grid(row=10, column=0, pady=15, sticky="w")
	...
	copy_btn.config(command=lambda: perform_copy_operation(entries, show_progress_bar))
